...
/**
 * An small bar indicating the title of the previous, current and next page to be shown in a ViewPager. Made to resemble
 * the indicator in the Google+ application in function.
 * 
...
public class ViewPagerIndicator extends RelativeLayout implements OnPageChangeListener {
    ...
    private void addContent() {
        int focusedColor = getResources().getColor(R.color.toolbar_text);
        int unfocusedColor = getResources().getColor(R.color.toolbar_text_unfocus);

        mFocusedTextColor = new int[]{Color.red(focusedColor), Color.green(focusedColor), Color.blue(focusedColor)};
        mUnfocusedTextColor = new int[]{Color.red(unfocusedColor), Color.green(unfocusedColor),
                Color.blue(unfocusedColor)};
    ...
    void updatePositions(int positionOffsetPixels) {
        ...
        if (positionOffsetPixels > 0) {
             ...
             * mPreviousGroup.setPadding(0, 0, Math.max(0, overlapLeft), 0); mNextGroup.setPadding(0, 0, 0, 0);
             */
        } else {
             ...
             * mNextGroup.setPadding(Math.max(0, overlapRight), 0, 0, 0); mPreviousGroup.setPadding(0, 0, 0, 0);
             */
        }
    }
    ...
    void updateColor(int offset) {
        ...
        float fraction;
        if (this.getWidth() != 0) {
            fraction = offset / ((float) this.getWidth() / 4.0f);
